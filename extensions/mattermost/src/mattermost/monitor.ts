      ...
      ws.on("message", async (data) => {
        ...
        if (payload.event === "reaction_added" || payload.event === "reaction_removed") {
          try {
            ...
            if (kind === "dm") {
              if (dmPolicy === "disabled") {
                logVerboseMessage(`mattermost: drop dm reaction (dmPolicy=disabled actor=${actorId})`);
                return;
              }
              if (dmPolicy !== "open" && !senderAllowed) {
                logVerboseMessage(`mattermost: drop dm reaction actor=${actorId} (dmPolicy=${dmPolicy})`);
                return;
              }
            } else {
              if (groupPolicy === "disabled") {
              ...
              if (groupPolicy === "allowlist") {
                ...
                if (!groupSenderAllowed) {
                  logVerboseMessage(`mattermost: drop group reaction actor=${actorId} (not in groupAllowFrom)`);
                  return;
                }
              }
            }
            ...
            const roomLabel = channelName ? `#${channelName}` : channelDisplay || `#${channelId}`;
            const emojiLabel = typeof emoji_name === "string" && emoji_name.trim()
              ? emoji_name.trim()
              : "emoji";
