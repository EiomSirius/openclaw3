...
describe("normalizeForwardedContext", () => {
  ...
  it("prefers author_signature from forward_origin over legacy forward_signature", () => {
    const ctx = normalizeForwardedContext({
      forward_origin: {
        type: "channel",
  ...
  it("extracts fromChatType from legacy forward_from_chat", () => {
    const ctx = normalizeForwardedContext({
      forward_from_chat: {
        title: "Super Group",
        id: -100555,
    ...
    expect(ctx).not.toBeNull();
    expect(ctx?.fromChatType).toBe("supergroup");
    expect(ctx?.fromType).toBe("legacy_chat");
