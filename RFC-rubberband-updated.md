...
- **Input sanitization** — Wrap user messages, escape shell chars ([#4542](https://github.com/openclaw/openclaw/pull/4542))
- **External frameworks** — Sentinel, prompt-guard ([#3345](https://github.com/openclaw/openclaw/pull/3345), [#4278](https://github.com/openclaw/openclaw/pull/4278))
...
└──────────────────────────────┘
       │
       ├─ BLOCK → notify user channel (optional), throw error
       ├─ ALERT → warn + optional channel notification
       └─ ALLOW → proceed
       │
       ▼
┌──────────────────────────────┐
...
Sample alert and blocks:
```
cat ~/.ssh/id_rsa         → ALERT (score 70, ssh_key_access)
...
|----------|--------------|---------------------|
| [#4542](https://github.com/openclaw/openclaw/pull/4542) (Input isolation) | Sanitize input, escape shell | Catches behavior if injection bypasses sanitization |
| [#4570](https://github.com/openclaw/openclaw/pull/4570) (Security Shield) | Rate limiting, firewall, IP blocking | Different layer (network vs exec pipeline) |
...
**New file:** `src/security/rubberband.ts` (~750 lines)
...
- Channel notification support via routeReply
...
tools:
  exec:
    rubberband:
      ...
      notifyChannel: true    # send BLOCK/ALERT to user's active channel
      thresholds:
        alert: 40
        block: 80
      allowedDestinations:   # optional whitelist for network targets
        - "github.com"
        - "api.openai.com"
```
